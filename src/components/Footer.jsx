...
export default function Footer() {
  return (
    <footer className="relative border-t border-white/[0.06]">
      <div className="w-full max-w-6xl mx-auto px-6 sm:px-8 py-16">
        <div className="grid md:grid-cols-3 gap-12 mb-16">
          <div className="md:col-span-2">
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="font-display font-semibold text-white text-lg tracking-tight">Tech Media</span>
            </div>
            <p className="text-gray-400 max-w-md leading-relaxed mb-6">
              Digital media and product company building content brands, AI-powered software, and the autonomous systems that connect them.
            </p>
          ...
          <div>
            <h4 className="font-display font-semibold text-white text-sm uppercase tracking-[0.15em] mb-5">Navigation</h4>
            <ul className="space-y-3">
              {footerLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="text-gray-400 hover:text-white transition-colors duration-300 text-sm">{link.label}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-white/[0.06] flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-gray-500 text-sm">&copy; {new Date().getFullYear()} AYM Tech Media. All rights reserved.</p>
          <p className="text-gray-600 text-xs">Built with craft. Shipped with conviction.</p>
        </div>
      </div>
    </footer>
  )
